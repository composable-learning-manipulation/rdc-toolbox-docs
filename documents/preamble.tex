...
\def\wtsymbol{22mm}
\def\wtunits{15mm}
\def\wtbounds{30mm} %{35mm} % % Если правое пол 15 - то 30, если первое поле 10, то 35
\def\wtcomment{18mm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\productcodefull}{СНАБ.01011} % Код организации + Регистрационный код
\newcommand{\programname}{Программа идентификации и синтеза динамических моделей манипуляционных роботов <<RDC Toolbox>>} % Название программы
\newcommand{\productname}{ идентификации и синтеза динамических моделей манипуляционных роботов <<RDC Toolbox>>} % Шифр (название) изделия
\newcommand{\productnamefirst}{Программа}
\newcommand{\productnamesecond}{<<RDC Toolbox>>}
\newcommand{\progr}{\programname}% Только для спецификации!
\newcommand{\progrcomp}{\programname}% Только для спецификации!

\title{\MakeUppercase\progr} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\firstapplication{АБВГ.123456.123} % Первое применение
\referencenumber{} % Справ №
% Установка титульников документа и ЛУ:
\def\titlemode{} % {} - короткий штамп, {1} - полный штамп, {2} - только перв. примен. {3} - только справ №
...
%
% СОГЛАСОВАНО 1. Гонноченко
\customerrank{Старший управляющий директор, \\ дир. центра Робототехники \\ ПАО Сбербанк } % На двух строках
\customername{А.С.~Гонноченко} % \phantom - убирает видимость

% УТВЕРЖДАЮ 1. Никифоров 2. Пыркин
\chiefconstructorrank{Проректор по научной работе \\Университет ИТМО \\ \phantom{some}} % Чтобы даты были на одном уровне
% \chiefconstructorrank{Проректор по научной работе \\Университет ИТМО } % Чтобы даты были на одном уровне
\chiefconstructorname{В.О.~Никифоров}
% \chiefconstructorrank{Главный конструктор\\изделия \productname\\начальник центра}
% \chiefconstructorname{А.Б.~Главный}
%
%
% Начальник подразделения (Научно-тематического центра)
\headofdepartmentrank{Руководитель проекта}
\headofdepartmentname{С.А~Колюбин}
%
%
% Исполнитель(и)
\authorname{В.А.~Антипов}
%%
%\authori{А.Б.~Пацак1}
%\authorii{А.Б.~Пацак2}

% \normocontrollerrank{Нормоконтролер}
% \normocontrollername{С.В.~Шаветов}

\year{2024}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Чтобы вернуть нунжно будет поправить espd.cls
